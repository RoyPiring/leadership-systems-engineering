...
The vault scaffolds three personas (a Master Coach Mentor, a resistant coachee named Marcus, and an ICF/iPEC Standards Gate that scores every session), a Self-ELI energy-baseline assessment, and the **4-Step Shift Arc** as the coaching engine. A 30-minute live session with Marcus produced a documented 9/10 ICF competency score, with presence, active listening, and awareness generation as the strongest competencies. The protocol then transfers out of simulation into seven days of calendared real conversations, and a Day-1-vs-Day-3 progression check.
